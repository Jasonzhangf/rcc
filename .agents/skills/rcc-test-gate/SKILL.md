...
3. 让错误输出显式指出缺失项。
4. 用验证结果决定任务是否能关闭。
5. 对需要 bundled/system config 的 host smoke / serve 验证，必须显式传入一个不存在的临时 `--config` 路径，禁止隐式读取用户 HOME 下现有 `~/.rcc/config.json`。
6. 对 loopback HTTP/SSE 测试，timeout 预算要能覆盖全量批次回归负载；避免 200ms 级超短超时导致假失败。
