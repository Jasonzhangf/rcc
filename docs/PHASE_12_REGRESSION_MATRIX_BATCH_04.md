# Phase 12 Regression Matrix Batch 04
...
## 目标
完成 **matrix 聚合 gate / 短路径统一回归入口**：

1. 本地一条命令能收口 batch01~03；
2. CI 复用同一入口；
3. 聚合层保持极薄，不复制任何业务断言。
...
1. 新增：
   ...
   - `scripts/verify_phase12_regression_matrix_batch04.sh`
2. 本批次只做：
   - 聚合 batch01~03 gate
   - 暴露统一短路径入口
   - 让 CI 可直接复用
...
- 当前实现阶段入口：`bash scripts/verify_phase12_regression_matrix_batch04.sh`
