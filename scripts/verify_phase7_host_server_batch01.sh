...
cleanup() {
  ...
  rm -f "$LOG_FILE" "$HEALTH_FILE" "$SMOKE_FILE"
}
trap cleanup EXIT
...
curl --silent --fail -X POST \
  ...
  -d '{"operation":"smoke","payload":"phase7-batch01"}' \
  "http://127.0.0.1:${PORT}/smoke" >"$SMOKE_FILE"

python3 - "$HEALTH_FILE" "$SMOKE_FILE" <<'PY'
import json
import pathlib
import sys
...
assert smoke['request']['payload'] == 'phase7-batch01', smoke
...
assert 'phase7-batch01' in smoke['response']['payload'], smoke
print('PHASE7_HOST_SERVER_BATCH01_HTTP: PASS')
PY
