...
cleanup() {
  ...
  rm -f "$LOG_FILE" "$HEALTH_FILE" "$SMOKE_FILE" "$REQUEST_FILE"
}
trap cleanup EXIT
...
curl --silent --fail -X POST \
  ...
  -d '{"operation":"smoke","payload":"phase7-batch02"}' \
...
python3 - "$HEALTH_FILE" "$SMOKE_FILE" "$REQUEST_FILE" <<'PY'
import json
import pathlib
import sys
...
assert smoke['request']['payload'] == 'phase7-batch02', smoke
...
assert 'phase7-batch02' in smoke['response']['payload'], smoke
...
print('PHASE7_HOST_SERVER_BATCH02_HTTP: PASS')
PY
