...
cleanup() {
  ...
  rm -f "$LOG_FILE" "$HEALTH_FILE" "$SMOKE_FILE" "$REQUEST_FILE" "$CHAT_FILE" "$RESPONSES_FILE"
}
trap cleanup EXIT
...
curl --silent --fail -X POST \
  ...
  -d '{"operation":"smoke","payload":"phase7-batch04"}' \
...
curl --silent --fail -X POST \
  ...
  "http://127.0.0.1:${PORT}/v1/responses" >"$RESPONSES_FILE"

python3 - "$HEALTH_FILE" "$SMOKE_FILE" "$REQUEST_FILE" "$CHAT_FILE" "$RESPONSES_FILE" <<'PY'
import json
import pathlib
import sys
...
responses = json.loads(pathlib.Path(sys.argv[5]).read_text())
...
assert smoke['request']['payload'] == 'phase7-batch04', smoke
...
assert 'phase7-batch04' in smoke['response']['payload'], smoke
...
assert responses['model'] == 'gpt-5', responses
assert responses['route']['target_block'] == 'pipeline', responses
assert responses['provider_runtime'] == 'noop-runtime', responses
assert responses['tool_plan']['scheduled'] == [], responses
assert responses['output'][0]['content'][0]['type'] == 'output_text', responses
assert 'operation=responses' in responses['output'][0]['content'][0]['text'], responses
print('PHASE7_HOST_SERVER_BATCH04_HTTP: PASS')
PY
