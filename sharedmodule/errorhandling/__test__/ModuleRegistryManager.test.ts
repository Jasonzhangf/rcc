...
describe('ModuleRegistryManager', () => {
  ...
  beforeEach(() => {
    ...
    mockResponseHandler = createMockResponseHandler();

    mockModuleRegistration = createMockModuleRegistration({
      moduleId: 'test-module',
      moduleName: 'TestModule'
    });

    mockDependencyModule = createMockModuleRegistration({
      moduleId: 'dependency-module',
      moduleName: 'DependencyModule'
    });

    // Setup default implementations
    mockResponseHandler.execute.mockResolvedValue(createMockErrorResponse({
  ...
  describe('Shutdown', () => {
    ...
    test('should handle shutdown errors gracefully', async () => {
      ...
      // Mock the shutdown method to throw an error
      const originalShutdown = moduleRegistryManager.shutdown;
      moduleRegistryManager.shutdown = jest.fn().mockRejectedValue(new Error('Clear failed'));

      try {
        await expect(moduleRegistryManager.shutdown()).rejects.toThrow('Clear failed');
      } finally {
        // Restore original method
        moduleRegistryManager.shutdown = originalShutdown;
      }
    });
  });
